...
__global__
void cufftShift_3D_slice_kernel(float* data, int N, int zIndex)
{
...
void cufftShift_3D_kernel(float* data, int N, dim3 gridSize, dim3 blockSize)
{
...
int main(int argc, char** argv)
{
    for (unsigned int N = INITIAL_SIZE_3D; N <= LIMIT_3D; INCREMENT)
    {
        for (unsigned int itr = 0; itr < NUM_ITERATIONS; itr++)
        {
            ...
            char profileName [2048] = "b_cufftShift_3D_Real";
            char N_char[1024];
            ...
            if (NUM_ITERATIONS == 1)
            {
                ...
                char fileName[1024];
                sprintf(fileName, "c_cufftShift_3D_Real__%dx%dx%d.check", NX, NY, NZ);
